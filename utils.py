...
def dice_coef_loss(logits, target, smooth=1e-6):
...
def bce_dice_loss(logits, target):
    ...
    dice = dice_coef_loss(logits, target)
    return bce + dice
...
def soft_dice_loss(logits, target, gamma=0.3, epsilon=1e-6):
    dice = dice_coeff(logits, target, epsilon=epsilon)  # [batch]
    log_dice = -torch.log(dice)       # log trên từng sample
    loss = torch.pow(log_dice, gamma) # (log_dice)^gamma
    return loss.mean()
